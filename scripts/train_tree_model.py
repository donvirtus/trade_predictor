...
class UniversalModelTrainer:
    def __init__(self, timeframe: str, models_dir: str = "models", pair: str = None, config = None,
        ...
        # Use path utility untuk generate database path
        self.db_path = generate_db_path(self.pair, timeframe)
    ...
    def train_xgboost(self, X_train, X_test, y_train, y_test):
        ...
        try:
            ...
            XGBRegressor = xgb.XGBRegressor  # type: ignore[attr-defined]
        except AttributeError:
            try:
                from xgboost.sklearn import XGBClassifier, XGBRegressor  # type: ignore
            except Exception as e:
                logger.error(f"Failed to import XGBClassifier: {e}")
                return None, 0.0

        if getattr(self, 'task_type', 'classification') == 'regression':
            # Use the proper Regressor for continuous targets
            model = XGBRegressor(
                n_estimators=800,
                max_depth=6,
                learning_rate=0.05,
    ...
    def save_model(self, model, model_type, metric_value, scaler, features):
        ...
        if self.overwrite:
            ...
            model_filename = generate_model_filename(
                self.pair, self.timeframe, model_type, target_suffix_clean, timestamp
            )
            scaler_filename = generate_model_filename(
                self.pair, self.timeframe, 'scaler', target_suffix_clean, timestamp
            )
        ...
        # Features filename menggunakan utility
        features_filename = generate_model_filename(
            self.pair, self.timeframe, model_type_lower, target_suffix_clean, 
            extension='_features.txt'
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Universal Model Training for Crypto Prediction (multi-horizon + extremes aware)')
    ...
    parser.add_argument('--pairs', type=str, nargs='+', default=None,
                       help='Train models for specific trading pairs (e.g., btcusdt ethusdt dogeusdt). If not specified, uses pairs from config.')
    ...
    try:
        ...
        if args.pairs:
            ...
            print(f"Pairs override: {args.pairs} → {pairs_to_process}")
        else:
            pairs_to_process = getattr(cfg, 'pairs', ['BTCUSDT'])

        print(f"Training for pairs: {pairs_to_process}")
        print(f"Training for timeframes: {tfs}")

        overall_ok = True
        for pair in pairs_to_process:
            print(f"\n🚀 Processing pair: {pair}")
            for tf in tfs:
                print(f"\n=== Training for {pair} {tf} ===")
                # Load dataset once to inspect available targets
                temp_trainer = UniversalModelTrainer(timeframe=tf, pair=pair, config_path=args.config, overwrite=args.overwrite, target_name=target_arg)
            ...
            # For each target, train for model(s)
            for tgt in ordered_targets:
                ...
                if model_name:
                    ...
                    if result:
                        model_name_display, metric_value = result
                        metric_str = f"{metric_value:.4f}" if trainer.task_type=='classification' else f"RMSE={metric_value:.6f}"
                        print(f"✅ {model_name_display} trained (target={tgt}) -> {metric_str}")
                    else:
                        print(f"❌ Training failed ({model_name} target={tgt})")
                        overall_ok = False
                else:
                    # Train all available models for this target
                    for candidate in ['lightgbm','xgboost','catboost']:
                        try:
                            result = trainer.train_single_model(candidate)
                            if result is None:
                                overall_ok = False
                        except Exception as e:
                            print(f"❌ {candidate} failed for target={tgt}: {e}")
                            overall_ok = False
